...
fdescribe('add a new user', () => {
  ...
  it('should click membership term', async () => {
    ...
    const mainWindowAndMembershipPopupWindowsNumber = 2;
    const handles = await browser.getAllWindowHandles();
    expect(mainWindowAndMembershipPopupWindowsNumber).toBe(handles.length);
  });

  it('should switch to pop up window', async () => {
    // 因為membershipPopup沒有Angular
  ...
  it('should click accept button', async () => {
    ...
    const handles = await browser.getAllWindowHandles();
    const mainWindowWindowsNumber = 1;
    expect(mainWindowWindowsNumber).toBe(handles.length);
  });
